...
VulkanBase::VulkanBase() :
    ...
    cpyBuffer(VK_NULL_HANDLE),
    cpyBufferMemory(VK_NULL_HANDLE),
    m_vkWaitSemaphore(VK_NULL_HANDLE),
    m_vkSignalSemaphore(VK_NULL_HANDLE)
{

    std::cout << " calling VulkanBase " << std::endl;
    // #ifdef NDEBUG
...
VkExternalMemoryHandleTypeFlagBits VulkanBase::getDefaultMemHandleType()
{
#ifdef _WIN64
...
void VulkanBase::createExternalSemaphore(VkSemaphore& semaphore, VkExternalSemaphoreHandleTypeFlagBits handleType)
{
...
void VulkanBase::run(void *handle, VkExternalMemoryHandleTypeFlagBits handleType, size_t size) {
    initWindow();
    initVulkan(handle, handleType, size);
    mainLoop();
    cleanup();
}
...
void VulkanBase::initVulkan(void *handle, VkExternalMemoryHandleTypeFlagBits handleType, size_t size) {
    createInstance();
    // setupDebugMessenger();
    createSurface();
    pickPhysicalDevice();
    ...
    createFramebuffers();
    createCommandPool();
    createTextureImage(handle, handleType, size,
...
void VulkanBase::cleanup() {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        vkDestroySemaphore(device, m_vkSignalSemaphore, nullptr);
        vkDestroySemaphore(device, m_vkWaitSemaphore, nullptr);
...
void VulkanBase::createInstance() {
    ...
    std::vector<const char*> LogDeviceExtensions = deviceExtensions;
...
void VulkanBase::createGraphicsPipeline() {
    auto vertShaderCode = readFile("/home/user1/Documents/works/Vulkan-cuda-Interop/shaders/vert.spv");
    auto fragShaderCode = readFile("/home/user1/Documents/works/Vulkan-cuda-Interop/shaders/frag.spv");
...
void VulkanBase::createTextureImage(void *handle, VkExternalMemoryHandleTypeFlagBits handleType, size_t size, 
    ...
    if (vkCreateBuffer(device, &bufferInfo, nullptr, &cpyBuffer) != VK_SUCCESS) {
        throw std::runtime_error("failed to create buffer!");
    }
    VkMemoryRequirements memRequirements;
    vkGetBufferMemoryRequirements(device, cpyBuffer, &memRequirements);
    ...
    if (vkAllocateMemory(device, &memAllocation, nullptr, &cpyBufferMemory) != VK_SUCCESS) {
    ...
    transitionImageLayout(textureImage, format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
        copyBufferToImage(cpyBuffer, textureImage, static_cast<uint32_t>(WIDTH), static_cast<uint32_t>(HEIGHT));
    transitionImageLayout(textureImage, format, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

    vkDestroyBuffer(device, cpyBuffer, nullptr);
    vkFreeMemory(device, cpyBufferMemory, nullptr);
}
...
uint32_t VulkanBase::findMemoryType(VkPhysicalDevice physicalDevice, uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
...
void VulkanBase::drawFrame() {
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR || framebufferResized) {
        std::cout << " subt optimal " << std::endl;
        framebufferResized = false;
        recreateSwapChain();
...
bool VulkanBase::checkDeviceExtensionSupport(VkPhysicalDevice device) {
    ...
    for (const auto& extension : availableExtensions) {
        // std::cout << " extension.extensionName : " << extension.extensionName << std::endl;
